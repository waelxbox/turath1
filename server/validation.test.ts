...
describe("Validation Portal - Round Robin Assignment", () => {
  ...
  it("should track line verdicts correctly", async () => {
    ...
    expect(submitLineVerdict).toHaveBeenCalledWith(expect.objectContaining({
      verdict: "correct",
      lineIndex: 0,
    }));
  });
  ...
  it("should complete an assignment", async () => {
    const { completeAssignment } = await import("./db");

    await completeAssignment(10, 20);
    expect(completeAssignment).toHaveBeenCalledWith(10, 20);
  });

  it("should return reviewer progress", async () => {
    const { getReviewerProgress } = await import("./db");

    const progress = await getReviewerProgress(1, "reviewer1");
    expect(progress.completed).toBe(2);
    expect(progress.totalAvailable).toBe(5);
  });

  it("should return validation stats with inter-rater agreement", async () => {
    const { getValidationStats } = await import("./db");

    const stats = await getValidationStats(1);
    expect(stats).not.toBeNull();
    expect(stats!.overallAccuracy).toBe(0.85);
    expect(stats!.interRaterAgreement).toBe(0.92);
    expect(stats!.uniqueReviewers).toBe(4);
  });
});

describe("Validation Portal - Session Management", () => {
  it("should find a session by valid token", async () => {
    const { getValidationSessionByToken } = await import("./db");

    const session = await getValidationSessionByToken("valid_token");
  ...
  it("should return null for invalid token", async () => {
    ...
    const session = await getValidationSessionByToken("invalid_token");
    expect(session).toBeNull();
  });

  it("should detect closed sessions", async () => {
    const { getValidationSessionByToken } = await import("./db");

    const session = await getValidationSessionByToken("closed_token");
    expect(session).not.toBeNull();

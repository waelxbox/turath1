...
/**
 * Cross-check an existing transcription using a DIFFERENT model.
 */
export async function crossCheckTranscription(
  project: { modelName: string; systemPrompt?: string | null; jsonSchema?: unknown },
  imageBase64: string,
  mimeType: string,
  existingJson: Record<string, unknown>
): Promise<CrossCheckResult> {
  const verifierModel = pickVerificationModel(project.modelName);

  const verificationPrompt = `You are a verification agent for archival document transcription.
You will be shown an image of a historical document AND an existing transcription (JSON).
Your job is to carefully re-read the image and compare your reading against the existing transcription.
...
INSTRUCTIONS:
1. Carefully read the document image
2. Compare each field in the existing transcription against what you see in the image
3. Identify any discrepancies, errors, or omissions
4. Rate the overall accuracy
...
Respond with a JSON object matching this exact schema:
{
  ...
  "corrections": [
    {
      "field": "<field name>",
      "original": "<what the transcription says>",
      "suggested": "<what you think it should say>",
      "severity": "low" | "medium" | "high",
...
If the transcription is accurate, return an empty corrections array with overallAssessment "accurate".
Only flag real errors you can see in the image — do not guess or hallucinate corrections.`;

  try {
    const response = await callLLM({
      messages: [
        { role: "system", content: "You are a meticulous archival document verification agent. Respond only with valid JSON." },
        {
          role: "user",
          content: [
      ...
      response_format: {
        ...
        json_schema: {
          ...
          schema: {
            type: "object",
            properties: {
              overallAssessment: { type: "string", enum: ["accurate", "minor_issues", "significant_issues"] },
              ...
              corrections: {
                ...
                items: {
                  ...
                  required: ["field", "original", "suggested", "severity", "reason"],
                  additionalProperties: false,
                },
              },
              summary: { type: "string" },
            },
            required: ["overallAssessment", "confidenceScore", "corrections", "summary"],
            additionalProperties: false,
          },
        },
      },
    ...
    if (!content) {
      return {
        ...
        confidenceScore: 0,
        corrections: [],
        summary: "Verification model returned no content.",
        modelUsed: verifierModel,
      };
    }

    const contentStr = typeof content === "string" ? content : JSON.stringify(content);
    const parsed = JSON.parse(contentStr) as Omit<CrossCheckResult, "modelUsed">;
    return { ...parsed, modelUsed: verifierModel };
  } catch (error) {
    return {
      overallAssessment: "minor_issues",
      confidenceScore: 0,

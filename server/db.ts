...
export type DocumentQuotaStatus = {
  ...
  plan: "free" | "owner" | "pro" | "team" | "enterprise";
...
/** Database-authoritative allowance; monthly paid usage never resets free lifetime usage. */
export async function getDocumentQuotaStatus(userId: number): Promise<DocumentQuotaStatus> {
  return (await import("./billing/quota")).readOrReserveQuota(userId);
}

/** Locks the account and reserves a slot before storage work. */
export async function reserveDocumentQuotaSlot(userId: number): Promise<DocumentQuotaStatus> {
  return (await import("./billing/quota")).readOrReserveQuota(userId, true);
}

/** Release exactly this upload's reservation, including across billing-period rollover. */
export async function releaseDocumentQuotaSlot(userId: number, reservationId?: string): Promise<void> {
  return (await import("./billing/quota")).releaseQuota(userId, reservationId);
}
